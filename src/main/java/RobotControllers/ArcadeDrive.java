...
public class ArcadeDrive implements RobotController {
    ...
    public void loop(JoysticksInterface joysticks, RobotInterface robot) {
        double leftVelocity = joysticks.getLeftStick().y + ((joysticks.getLeftStick().x + Math.abs(joysticks.getLeftStick().x)) / 2) * 2.5; // get the velocity of the left half of the robot
        double rightVelocity = joysticks.getLeftStick().y + Math.abs((joysticks.getLeftStick().x - Math.abs(joysticks.getLeftStick().x)) / 2) * 2.5; // get the velocity of the right half of the robot
